...
@test LibML.AccuracyScore(yhat, y) == 0.49f0 || error("AccuracyScore failed")
...
@test LibML.PPVScore(yhat, y) == 0.5f0 || error("PPVScore failed")
@test LibML.RecallScore(yhat, y) == 0.47058824f0 || error("RecallScore failed")
@test LibML.TPRScore(yhat, y) == 0.47058824f0 || error("TPRScore failed")
